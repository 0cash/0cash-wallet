...
public class StartupProgressDialog extends JFrame {
    ...
    public StartupProgressDialog(ZCashClientCaller clientCaller)
    {
        ...
        URL iconUrl = this.getClass().getClassLoader().getResource("images/0cash.png");
    ...
    public void waitForStartup() throws IOException,
        ...
        Log.info("Splash: checking if 0cashd is already running...");
        ...
        if (!shouldStartZCashd) {
        	Log.info("Splash: 0cashd already running...");
            // What if started by hand but taking long to initialize???
//            doDispose();
//            return;
        } else
        {
        	Log.info("Splash: 0cashd will be started...");
        }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread() {
            public void run() {
            	Log.info("Stopping 0cashd because we started it - now it is alive: " +
                ...
                try
                {
	                ...
	                while (!StartupProgressDialog.this.waitFor(daemonProcess, 3000))
	                {
	                	...
	                	Log.info("Waiting for " + ((end - start) / 1000) + " seconds for 0cashd to exit...");
	                ...
	                if (StartupProgressDialog.this.isAlive(daemonProcess)) {
	                	Log.info("0cashd is still alive although we tried to stop it. " +
	                                           "Hopefully it will stop later!");
	                    } else
	                    	Log.info("0cashd shut down successfully");
                } catch (Exception bad) {
                	Log.error("Couldn't stop 0cashd!", bad);
                }
            }
        });
